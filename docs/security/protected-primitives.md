...
`jury-protected` uses the maintained local provider at
`third_party/sanitization/crates/sanitization` (package 2.0.4), with MIT OR
Apache-2.0 licensing. It was imported from the owned `featherenvy/sanitization`
fork at Git revision `3f0a72c5640b4919dec93799725c7573b2878a8c`.
Current provider bytes are bound by Jury's Git history and release source
hashes; the import revision records ancestry, not subsequent local changes.
The fork is based on upstream v2.0.4 commit
...
provider; Jury's application crates do not contain native FFI. These are solo source review and
behavior tests, not independent review.

The provider is developed in this monorepo as a separate Cargo workspace.
No provider Git fetch is needed for builds. Original source, tests, and licenses
are retained; external registry dependencies still need fetching or vendoring.
Registry advisories do not automatically cover this path dependency. A clean
`cargo audit` result does not establish that upstream advisories were checked
against the maintained source. Provider updates require checking upstream
advisories and the local delta explicitly. The upstream version and import
ancestry are recorded in the provider's `workspace.metadata.jury` table.
All Jury and maintained provider crates disable registry publication.
