...
int HpkeOpen(char** arguments) {
  ...
  if (key == nullptr || context == nullptr ||
      ...
      !EVP_HPKE_CTX_setup_recipient(
          context, key, EVP_hpke_hkdf_sha256(), EVP_hpke_aes_256_gcm(),
...
// Public deterministic fixture generation for two-way provider comparison.
int HpkeSeal(char** arguments) {
  std::vector<uint8_t> private_seed, seed, info, aad, plaintext;
  if (!Decode(arguments[2], &private_seed) || !Decode(arguments[3], &seed) ||
      !Decode(arguments[4], &info) || !Decode(arguments[5], &aad) ||
      !Decode(arguments[6], &plaintext)) return 2;
  EVP_HPKE_KEY* key = EVP_HPKE_KEY_new();
  EVP_HPKE_CTX* context = EVP_HPKE_CTX_new();
  std::vector<uint8_t> public_key(EVP_HPKE_MAX_PUBLIC_KEY_LENGTH);
  size_t public_size = 0;
  std::vector<uint8_t> enc(EVP_HPKE_MAX_ENC_LENGTH);
  size_t enc_size = 0;
  if (!key || !context ||
      !EVP_HPKE_KEY_init(key, EVP_hpke_xwing(), private_seed.data(), private_seed.size()) ||
      !EVP_HPKE_KEY_public_key(key, public_key.data(), &public_size, public_key.size()) ||
      !EVP_HPKE_CTX_setup_sender_with_seed_for_testing(context, enc.data(), &enc_size, enc.size(),
          EVP_hpke_xwing(), EVP_hpke_hkdf_sha256(), EVP_hpke_aes_256_gcm(),
          public_key.data(), public_size, info.data(), info.size(), seed.data(), seed.size())) {
    EVP_HPKE_CTX_free(context); EVP_HPKE_KEY_free(key); return 3;
  }
  std::vector<uint8_t> ciphertext(plaintext.size() + 16);
  size_t ciphertext_size = 0;
  const int result = EVP_HPKE_CTX_seal(context, ciphertext.data(), &ciphertext_size, ciphertext.size(),
      plaintext.data(), plaintext.size(), aad.data(), aad.size());
  EVP_HPKE_CTX_free(context); EVP_HPKE_KEY_free(key);
  if (result != 1) return 4;
  for (size_t index = 0; index < enc_size; ++index) std::printf("%02x", enc[index]);
  std::printf("\n");
  for (size_t index = 0; index < ciphertext_size; ++index) std::printf("%02x", ciphertext[index]);
  std::printf("\n");
  return 0;
}

int HpkeReject(char** arguments, const EVP_HPKE_AEAD* algorithm) {
  std::vector<uint8_t> private_key, encapsulation, info, aad, ciphertext;
  if (!Decode(arguments[2], &private_key) ||
      !Decode(arguments[3], &encapsulation) || !Decode(arguments[4], &info) ||
      !Decode(arguments[5], &aad) || !Decode(arguments[6], &ciphertext)) {
  ...
  if (!EVP_HPKE_KEY_init(key, EVP_hpke_xwing(), private_key.data(),
                         private_key.size()) ||
      !EVP_HPKE_CTX_setup_recipient(
          context, key, EVP_hpke_hkdf_sha256(), algorithm,
          encapsulation.data(), encapsulation.size(), info.data(), info.size())) {
    EVP_HPKE_CTX_free(context);
    EVP_HPKE_KEY_free(key);
    return 0;
...
int main(int argc, char** argv) {
  if (argc == 7 && std::strcmp(argv[1], "hpke-seal") == 0) return HpkeSeal(argv);
  ...
  if (argc == 7 && std::strcmp(argv[1], "hpke-reject") == 0)
    return HpkeReject(argv, EVP_hpke_aes_256_gcm());
  if (argc == 7 && std::strcmp(argv[1], "hpke-reject-chacha") == 0)
    return HpkeReject(argv, EVP_hpke_chacha20_poly1305());
