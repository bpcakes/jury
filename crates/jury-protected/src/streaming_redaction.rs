...
pub const MAX_REDACTION_PATTERNS: usize = 4_096;
pub const MAX_REDACTION_PATTERN_BYTES: usize = 16 * 1024 * 1024;
pub const MAX_REDACTION_PATTERN_LEN: usize = 512 * 1024;
pub const MAX_OUTPUT_CHUNK_LEN: usize = 64 * 1024;

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum StreamingRedactorError {
    PatternCount,
    PatternBytes,
    PatternLength,
    PatternTooShort,
...
impl fmt::Display for StreamingRedactorError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        let message = match self {
            Self::PatternCount => "redaction pattern count exceeds the supported bound",
...
impl std::error::Error for StreamingRedactorError {}
...
impl StreamingRedactor {
    pub fn from_patterns(
        patterns: impl IntoIterator<Item = Vec<u8>>,
    ) -> Result<Self, StreamingRedactorError> {
        Self::new(patterns.into_iter().map(Zeroizing::new).collect())
    }

    pub fn from_redactors<'a>(
        redactors: impl IntoIterator<Item = &'a crate::Redactor>,
    ) -> Result<Self, StreamingRedactorError> {
    ...
    fn new(mut patterns: Vec<Zeroizing<Vec<u8>>>) -> Result<Self, StreamingRedactorError> {
        ...
        let matcher = if patterns.is_empty() {
            ...
            Some(Arc::new(
                AhoCorasickBuilder::new()
                    ...
                    .map_err(|_| StreamingRedactorError::Matcher)?,
            ))
        };
        let pending_capacity = max_pattern_len
            .saturating_sub(1)
            .checked_add(MAX_OUTPUT_CHUNK_LEN)
            .ok_or(StreamingRedactorError::PatternBytes)?;
        Ok(Self {
            matcher,
            pattern_count,
            max_pattern_len,
    ...
    pub fn independent_stream(&self) -> Self {
        Self {
    ...
    pub fn push_chunk<W: Write + ?Sized>(
        &mut self,
        chunk: &[u8],
        writer: &mut W,
    ) -> io::Result<()> {
        if chunk.len() > MAX_OUTPUT_CHUNK_LEN {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                format!("output chunk exceeds the {MAX_OUTPUT_CHUNK_LEN} byte matcher bound"),
            ));
        }
    ...
    pub fn finish<W: Write + ?Sized>(mut self, writer: &mut W) -> io::Result<()> {
        match &self.matcher {
            Some(matcher) => {
...
fn validate_patterns(patterns: &[Zeroizing<Vec<u8>>]) -> Result<(), StreamingRedactorError> {
    ...
    for pattern in patterns {
        if pattern.len() < MIN_REDACTABLE_LEN {
            return Err(StreamingRedactorError::PatternTooShort);
        }
        if pattern.len() > MAX_REDACTION_PATTERN_LEN {
            return Err(StreamingRedactorError::PatternLength);
        }
        total = total
            .checked_add(pattern.len())
            .ok_or(StreamingRedactorError::PatternBytes)?;
        if total > MAX_REDACTION_PATTERN_BYTES {
            return Err(StreamingRedactorError::PatternBytes);
        }
    }
    Ok(())
}
...
fn emit_matches<W: Write + ?Sized>(
    ...
    for matched in matcher.find_iter(input) {
        ...
        writer.write_all(REDACTION_MARKER)?;
        cursor = matched.end();
...
mod tests {
    ...
    fn redact_with_chunks(
        patterns: &[&[u8]],
        input: &[u8],
        chunks: &[usize],
    ) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        let mut redactor = StreamingRedactor::new(self::patterns(patterns))?;
        let mut output = Vec::new();
        let mut cursor = 0;
        for &chunk_len in chunks {
            let end = (cursor + chunk_len).min(input.len());
            redactor.push_chunk(&input[cursor..end], &mut output)?;
            cursor = end;
        }
        if cursor < input.len() {
            redactor.push_chunk(&input[cursor..], &mut output)?;
        }
    ...
    #[test]
    fn redacts_matches_split_at_every_byte_boundary() -> Result<(), Box<dyn std::error::Error>> {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let expected = [&[0xff][..], REDACTION_MARKER, &[0xfe, 0x80][..]].concat();
        for split in 0..=input.len() {
            assert_eq!(
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        let mut stdout = StreamingRedactor::new(patterns(&[b"secret-value"]))?;
        let mut stderr = stdout.independent_stream();
        let mut stdout_bytes = Vec::new();
        let mut stderr_bytes = Vec::new();
        stdout.push_chunk(b"secret", &mut stdout_bytes)?;
        stderr.push_chunk(b"-value", &mut stderr_bytes)?;
        stdout.finish(&mut stdout_bytes)?;
        stderr.finish(&mut stderr_bytes)?;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        let needle = vec![b'n'; MAX_REDACTION_PATTERN_LEN];
        let mut redactor = StreamingRedactor::new(vec![Zeroizing::new(needle)])?;
        let mut output = Vec::new();
        for _ in 0..4 {
            redactor.push_chunk(&vec![b'x'; MAX_OUTPUT_CHUNK_LEN], &mut output)?;
            assert!(redactor.pending_len() < MAX_REDACTION_PATTERN_LEN);
        }
        Ok(())
    }
    ...
    fn constructor_enforces_count_total_length_and_minimum_bounds() {
        let too_many = (0_usize..=MAX_REDACTION_PATTERNS)
        ...
        let total_overflow = (0..=MAX_REDACTION_PATTERN_BYTES / MAX_REDACTION_PATTERN_LEN)
            .map(|index| {
                let mut pattern = vec![b'x'; MAX_REDACTION_PATTERN_LEN];
    ...
    -> Result<(), StreamingRedactorError> {
        let redactor =
            StreamingRedactor::new(patterns(&[b"duplicate-secret", b"duplicate-secret"]))?;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        let mut redactor = StreamingRedactor::new(patterns(&[b"secret-value"]))?;
        let mut output = Vec::new();
        let error = redactor
            .push_chunk(&vec![b'x'; MAX_OUTPUT_CHUNK_LEN + 1], &mut output)
            .err()
            .ok_or("oversized chunk should fail")?;
