...
fn read_http_request(stream: &mut TcpStream) -> Result<(String, Vec<u8>, bool), String> {
    ...
    // Accepted sockets can inherit the listener's nonblocking mode on Darwin.
    // This parser owns bounded blocking I/O; set that mode explicitly.
...
fn run_with_async_approval(
    ...
    let prepared = (|| {
        ...
        publish_approval(approval_path, &artifact, context.policy, context.approver)
    })();
    let review = match prepared {
        Ok(review) => review,
        Err(error) => {
            child.kill()?;
            let output = child.wait_with_output()?;
            let diagnostic = serde_json::from_slice::<serde_json::Value>(&output.stderr)
                .ok()
                .and_then(|value| value["error"]["code"].as_str().map(str::to_owned));
            return Err(format!("{error}; CLI error code: {diagnostic:?}").into());
        }
    };
...
#[test]
fn http_fixture_waits_for_delayed_bytes_on_an_accepted_socket() -> TestResult {
    let listener = TcpListener::bind("127.0.0.1:0")?;
    let mut client = TcpStream::connect(listener.local_addr()?)?;
    let (mut server, _) = listener.accept()?;
    // Darwin can inherit nonblocking mode from the listener. Force that
    // starting state on both platforms so the regression is deterministic.
    ...
    let early = completed.recv_timeout(Duration::from_millis(50));
    let written = client.write_all(format!(
        "POST /v1/requests/reserve HTTP/1.1\r\nContent-Length: 2\r\nAuthorization: Bearer {CLIENT_TOKEN}\r\n\r\n{{}}"
    ).as_bytes());
    let result = worker.join().map_err(|_| "HTTP reader panicked")?;
    assert!(matches!(early, Err(std::sync::mpsc::RecvTimeoutError::Timeout)),
        "HTTP reader finished before the client sent its request: {result:?}");
    written?;
    let (path, body, authorized) = result?;
    assert_eq!(path, "/v1/requests/reserve");
    assert_eq!(body, b"{}");
    assert!(authorized);
    Ok(())
}
