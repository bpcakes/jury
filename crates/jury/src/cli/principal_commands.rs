...
include!("principal_commands/owner_change.rs");
...
#[cfg(test)]
mod owner_label_tests {
    use super::*;

    #[test]
    fn label_expiry_rejects_the_exact_boundary_without_extending_it() {
        assert!(require_owner_label_unexpired(None, u64::MAX).is_ok());
        assert!(require_owner_label_unexpired(Some(101), 100).is_ok());
        for now in [101, 102] {
            let error = require_owner_label_unexpired(Some(101), now).err();
            assert_eq!(
                error.map(|error| error.code()),
                Some("owner-change-label-expired")
            );
        }
    }
}
