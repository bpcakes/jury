...
#[cfg(any(target_os = "linux", target_os = "macos"))]
#[test]
fn expired_drain_wait_still_reads_ready_bytes_and_eof() -> Result<(), Box<dyn std::error::Error>> {
    let mut child = Command::new("/bin/sh")
        .args(["-c", "printf ExampleReadyOutput"])
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()?;
    let drains = OwnedProcessOutputDrains::start(&mut child, ProcessOutputLimits::default(), None)?;
    assert!(child.wait()?.success());
    // No waiting budget remains, but already-ready bytes/EOF still need one
    // bounded nonblocking poll. This is separate from open-writer timeout tests.
    let (stdout, _) = drains.finish(Duration::ZERO, &mut IgnoreProcessActivity)?;
    let stdout = stdout.ok_or("stdout was not captured")?;
    assert!(stdout.complete);
...
#[cfg(any(target_os = "linux", target_os = "macos"))]
#[test]
fn expired_drain_redacts_before_capture_and_observation() -> Result<(), Box<dyn std::error::Error>> {
    #[derive(Default)]
    ...
    let mut child = Command::new("/bin/sh")
        .args(["-c", "printf ExampleSecret:Example"])
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()?;
    let redactor = StreamingRedactor::from_patterns([b"ExampleSecret".to_vec()])?;
    let drains = OwnedProcessOutputDrains::start(
        &mut child,
        ProcessOutputLimits::default(),
        Some(ProcessOutputRedaction::new(redactor)),
    )?;
    ...
    let stdout = stdout.ok_or("stdout was not captured")?;
    assert!(stdout.complete);
    // The trailing potential prefix must be flushed at EOF, while the full
    // match must never reach either capture or observation unredacted.
